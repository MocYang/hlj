...
export const createMap = {
  ...
  flawto(view3d, location) {
    const pos = {
      ...
      z: Number(location.z),
      pitch: 80, // 俯仰角 0——90度
      yaw: Number(location.yaw), // 偏航角 0-360度
      roll: 0 // 翻滚角
    }
    ...
    view3d.SetMouseCallback((res) => {
      ...
      if (Array.isArray(res)) {
        ...
        if (res.typename === 'model') {
          ...
          console.log(res.gid, 'res.gid111222')
...
export const Event = {
  ...
  videoModels: [],
  // 视频投影
  videoProjection2(view3d, objs) {
    this.setTreeVisible(view3d, false)
    console.log('打开视频投地', objs)
    let video = JSON.parse(objs[0].globalVideoMo)
    video.gid = 'GLOBALCAM_' + this.videoModelId++
    if (!video) return
    let icons = objs
      .filter((x) => x.location)
      .map((x) => {
        return {
          gid: 'CAM_' + x.deviceCode,
          type: 'image', // 10102  或  image
          style: 'qiangji_icon', // 立柱的颜色样式
          screen: true,
          scale: 0.5, // 整个对象的缩放比例
          location: JSON.parse(x.location),
          attr: {
            category_name: '摄像机',
            device_code: x.deviceCode
          }
        }
      })
    console.log('绘制icons', icons)
    Map.runSync({
      func() {
        return Map.overLayerCreateObject(view3d, video)
      },
      success() {
        Event.videoModels.push(video)
        Map.runSync({
          func() {
            return Map.overLayerUpdateObject(view3d, video)
          }
        })
      }
    })
    Map.runSync({
      func() {
        return Map.overLayerCreateObjects(view3d, icons)
      },
      success() {
        Event.videoModels.push(...icons)
      }
    })
  }
}
...
export const Map = {
  ...
  flyTo(map, pos, distance = 300, isBefore = true) {
    if (pos.x && pos.y && pos.z) {
      ...
      Map.runSync({
        func() {
          return Map.flyToPosition(window.$map_light, posNew)
        }
      })
    } else {
  ...
  overLayerRemoveObjectById(map, id) {
    return new Promise((resolve) => {
      map.OverLayerRemoveObjectById(id)
      setTimeout(() => {
        resolve()
      }, 100)
    })
  ...
  setObjectHighlight(map, gid) {
    return new Promise((resolve) => {
      setTimeout(() => {
        map.SetObjectHighlight(gid, (res) => {})
        resolve({})
      }, 100)
    })
  },
  ...
  resetAllBuildings(map) {
    return new Promise((resolve) => {
      setTimeout(() => {
        map.ResetAllBuildings()
        resolve()
      }, 100)
    })
  }
